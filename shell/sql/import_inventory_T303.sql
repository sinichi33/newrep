...
UPDATE
	...
	INNER JOIN eav_attribute_option_value eaov ON eao.`option_id` = eaov.`option_id` AND eaov.value = 'R120'
	INNER JOIN temp_SAPInventoryImport ti ON cp.sku = ti.sku
	LEFT OUTER JOIN temp_ReservedItems tri ON ti.sku = tri.sku AND tri.`store_code` = 'T303'
SET ast.`quantity_in_stock` = ti.`qty` - IFNULL(tri.qty,0)
WHERE pos.store_code = 'T303'
;
