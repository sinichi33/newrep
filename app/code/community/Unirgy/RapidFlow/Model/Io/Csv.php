...
class Unirgy_RapidFlow_Model_Io_Csv extends Unirgy_RapidFlow_Model_Io_File
{
    ...
    public function read()
    {
        ...
        /* ICUBE CUSTOM - move read files to archive folder */
        $meta_data = stream_get_meta_data($this->_fp);
        $filename = str_replace('-import.log','',$meta_data['uri']);
        $path = Mage::getBaseDir('var').Mage::getStoreConfig('urapidflow/dirs/archive_dir', $this->getStoreId());
        if(empty($path)) {
            $path = Mage::getBaseDir('var').'/urapidflow/import/archive';
        }
        Mage::getConfig()->createDirIfNotExists($path);
        rename($filename, $path.'/'.basename($filename));
        /* END OF ICUBE CUSTOM */
        return $result;
    }
