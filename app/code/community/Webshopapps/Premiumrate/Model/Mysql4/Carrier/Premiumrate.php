...
class Webshopapps_Premiumrate_Model_Mysql4_Carrier_Premiumrate extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function getNewRate(Mage_Shipping_Model_Rate_Request $request)
    {
		...
		/* Icube update - set City into city+kecamatan */
		 if(Mage::app()->getFrontController()->getRequest()->getRouteName() == 'adminhtml')
		 {
  $kecamatan = Mage::getSingleton('adminhtml/session_quote')->getQuote()->getShippingAddress()->getKecamatan();
  		}
  		else {
	  		$kecamatan = Mage::getSingleton('checkout/session')->getQuote()->getShippingAddress()->getKecamatan();
  		}
  		$newCity = $request->getDestCity().'-'.$kecamatan;
		$this->_request->setDestCity($newCity);
		/* End of Icube */
