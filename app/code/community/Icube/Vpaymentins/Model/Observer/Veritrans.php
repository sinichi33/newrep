<?php
class Icube_Vpaymentins_Model_Observer_Veritrans
{
...
//                Mage::log("order id:".$order_id."|transaction_id:".$transaction_id."|vt_status:".$vt_status, null, 'transaction_id.log', true);
            ...
            if(Mage::getStoreConfig('payment/vpaymentins/authorize_only') || $vt_status == 'challenge') {
                ...
                $sentReq  = Mage::helper('vpaymentins')->sentReqVtransAuthorize($comidity);
...
//                                Mage::log('VD06', null, 'logging_INSERT.log', true);
    ...
    public function checkAuthorizeOrder(Varien_Event_Observer $observer)
    {
        ...
        if ($order->getPayment()->getMethodInstance()->getCode() == 'vpaymentins') {
            if(Mage::getStoreConfig('payment/vpaymentins/authorize_only')) {
                $order->setStatus(Icube_Vpayment_Model_Vpaymentins::STATE_PENDING);
                $order->save();
            }
        }
        return $this;
...
//            Mage::log("transaction_id:".$transaction_id, null, 'transaction_id.log', true);
            $result = Mage::helper('vpaymentins')->sendCancelTransaction($transaction_id);
