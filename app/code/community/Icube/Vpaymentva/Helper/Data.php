...
class Icube_Vpaymentva_Helper_Data extends Mage_Core_Helper_Abstract
{
    public function sentReqVtrans($comidity)
    {
        $json       = json_encode($comidity);
        $server_key = Mage::getStoreConfig('payment/vpaymentva/server');
        $server_key = base64_encode($server_key);
//        $server_key = base64_encode($this->getServerKey());
//        $url        = $this->getUrl();
        $url        = Mage::getStoreConfig('payment/vpaymentva/vurl');
        $ch         = curl_init($url);
    ...
    public function sendCancelTransaction($order_id)
    {
        $server_key = Mage::getStoreConfig('payment/vpaymentva/server');
        $server_key = base64_encode($server_key);
        $url        = Mage::getStoreConfig('payment/vpaymentva/cancel_url');
...
//        Mage::log(json_decode($result),null,'sendCancelTransaction.log',true);
    ...
    public function getVeritransJs() {
        $mode = Mage::getStoreConfig('payment/vpaymentva/vt_mode');
        if($mode == Icube_Vpaymentva_Model_Resource_Mode::MODE_DEVELOPMENT) {
            return '<script type="text/javascript" src="'.Mage::getDesign()->getSkinUrl('js/veritrans.min.js').'"></script>';
        } elseif ($mode == Icube_Vpaymentva_Model_Resource_Mode::MODE_PRODUCTION) {
