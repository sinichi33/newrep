...
class Fishpig_AttributeSplash_Model_Layer extends Mage_Catalog_Model_Layer 
{
	...
	*/
	...
	/**
	 * Vtr Retrieve the product collection for the Splash Page exclude some entity_id
	 *
	 * @param entity_id that you want to exclude
	 * @return
	 */
	public function getProductCollectionFilter($params)
	 {
		$key = 'splash_' . $this->getSplashPage()->getId();
		...
		if (isset($this->_productCollections[$key])) {
			...
			// exclude by entity_id
			$collection->addAttributeToFilter('entity_id', array('nin' => $params));
		}
		else {
			$collection = $this->getSplashPage()->getProductCollection();
			$this->prepareProductCollection($collection);
