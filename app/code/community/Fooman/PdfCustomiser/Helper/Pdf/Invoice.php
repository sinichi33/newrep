...
class Fooman_PdfCustomiser_Helper_Pdf_Invoice extends Fooman_PdfCustomiser_Helper_Pdf
{
     ...
     * @access public
     */
    public function getPdfColumns($type = '')
    {
        $columns = ($type) ? $type : 'invoicecolumns';
        if (!isset($this->_parameters[$this->getStoreId()][$columns])) {
            $this->_parameters[$this->getStoreId()][$columns] =
                Mage::getStoreConfig('sales_pdf/invoice/'.$columns, $this->getStoreId());
        }
        return $this->_parameters[$this->getStoreId()][$columns];
    }
