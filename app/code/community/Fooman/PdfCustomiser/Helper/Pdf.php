...
abstract class Fooman_PdfCustomiser_Helper_Pdf extends Mage_Core_Helper_Abstract
{
     ...
     * @access public
     */
    public function getPdfColumnHeaders($type = '')
    {
        $columnsToPrint = explode(',', $this->getPdfColumns($type));
        ...
        $columnTitles = array(
            ...
            // ICUBE's custom
            'discount' => htmlentities(Mage::helper('sales')->__('Disc. Amount'), ENT_QUOTES, 'UTF-8', false), 
            'price_after_disc' => htmlentities(Mage::helper('sales')->__('Price Aft. Disc'), ENT_QUOTES, 'UTF-8', false), 
            'handling_fee_item' => htmlentities(Mage::helper('sales')->__('Handling Fee'), ENT_QUOTES, 'UTF-8', false), 
            'pos_price' => htmlentities(Mage::helper('sales')->__('POS Price'), ENT_QUOTES, 'UTF-8', false), 
    ...
    public function OutputCustomerOrderComment()
    {
        ...
        } elseif ($order->getCustomerNote() || $order->getCustomerNotes()) {
            $orderComments[] = array(
    ...
    public function fixEncoding($input)
    {
        if(is_array($input)){
            ...
            foreach ($input as $string){
                $returnArray[] = $this->fixEncoding($string);
            }
            return $returnArray;
        }
