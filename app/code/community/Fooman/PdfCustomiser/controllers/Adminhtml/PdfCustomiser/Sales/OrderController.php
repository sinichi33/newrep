...
class Fooman_PdfCustomiser_Adminhtml_PdfCustomiser_Sales_OrderController extends Fooman_EmailAttachments_Admin_OrderController
{
    ...
    public function pdfordersAction()
    {
        ...
        if (sizeof($orderIds)) {
            Mage::getModel('pdfcustomiser/order')->renderPdf(null, $orderIds, null, false, null, $forcedStoreId);
        } else {
        ...
        foreach ($orderIds as $orderId) {
            if ($i == 1) {
                $pdf = Mage::getModel('pdfcustomiser/invoice')->renderPdf(
                    null, array($orderId), null, true, null, $forcedStoreId
                );
            } else {
                $pdf = Mage::getModel('pdfcustomiser/invoice')->renderPdf(
                    null, array($orderId), $pdf, true, null, $forcedStoreId
                );
            }
            if ($i == $total) {
                $pdf = Mage::getModel('pdfcustomiser/shipment')->renderPdf(
                    null, array($orderId), $pdf, false, 'orderDocs_', $forcedStoreId
                );
                $this->_redirectReferer('adminhtml/sales_order');
            } else {
                $pdf = Mage::getModel('pdfcustomiser/shipment')->renderPdf(
    ...
    public function printAction()
    {
        ...
        $forcedStoreId = $this->getRequest()->getParam('force_store_id');
        if ($orderId) {
            Mage::getModel('pdfcustomiser/order')->renderPdf(null, array($orderId), null, false, null, $forcedStoreId);
        } else {
    ...
    public function pdfshipmentAction()
    {
        ...
        if ($orderId) {
            if (!Fooman_PdfCustomiser_Model_Abstract::COMPAT_MODE) {
                Mage::getModel('pdfcustomiser/shipment')->renderPdf(null, array($orderId), null, false, null, $forcedStoreId);
            } else {
                $pdf = Mage::getModel('sales/order_pdf_shipment')->getPdf(null, array($orderId), null, false, null, $forcedStoreId);
                $print = $pdf->render();
                if ($print) {
            ...
            if ($shipment) {
                Mage::getModel('pdfcustomiser/shipment')->renderPdf(array($shipment), null, null, false, null, $forcedStoreId);
            } else {
                $addError = true;
            }
        } else {
    ...
    public function pdfinvoiceAction()
    {
        ...
        if ($orderId) {
            ...
            if ($invoice) {
                Mage::getModel('pdfcustomiser/invoice')->renderPdf(array($invoice), null, null, false, null, $forcedStoreId);
            } else {
                $addError = true;
            }
        } else {
    ...
    public function pdfcreditmemoAction()
    {
        ...
        $addError = false;
        if ($orderId) {
            Mage::getModel('pdfcustomiser/order')->renderPdf(null, array($orderId), null, false, '', $forcedStoreId);
        } elseif ($creditmemo) {
            $creditmemo = Mage::getModel('sales/order_creditmemo')->load($creditmemo);
            if ($creditmemo) {
                Mage::getModel('pdfcustomiser/creditmemo')->renderPdf(array($creditmemo), null, null, false, '', $forcedStoreId);
            } else {
                $addError = true;
            }
        } else {
    ...
    public function pdfdocsAction()
    {
        ...
        if (sizeof($orderIds)) {
            if (!Fooman_PdfCustomiser_Model_Abstract::COMPAT_MODE) {
                $pdf = Mage::getModel('pdfcustomiser/invoice')->renderPdf(null, $orderIds, null, true, null, $forcedStoreId);
                $pdf = Mage::getModel('pdfcustomiser/shipment')->renderPdf(null, $orderIds, $pdf, true, null, $forcedStoreId);
                $pdf = Mage::getModel('pdfcustomiser/order')->renderPdf(null, $orderIds, $pdf, true, null, $forcedStoreId);
                $pdf = Mage::getModel('pdfcustomiser/creditmemo')->renderPdf(null, $orderIds, $pdf, false, 'orderDocs_', $forcedStoreId);
                $this->_redirectReferer('adminhtml/sales_order');
    ...
    public function pdfpickinglistAction()
    {
        ...
        $forcedStoreId = $this->getRequest()->getParam('force_store_id');
        if (sizeof($orderIds)) {
            Mage::getModel('pdfcustomiser/order_pickingList')->renderPdf(null, $orderIds, null, false, null, $forcedStoreId);
        } else {
            $this->_getSession()->addError($this->__('There are no printable documents related to selected orders'));
        }
        $this->_redirectReferer('adminhtml/sales_order');
