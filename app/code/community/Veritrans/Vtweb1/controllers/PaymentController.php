 ...
 * http://yoursite.com/vtweb1/payment/notification
 */

require_once(Mage::getBaseDir('lib') . '/veritrans-php/Veritrans.php');

class Veritrans_Vtweb1_PaymentController
  ...
  public function redirectAction() {
    ...
    $order->setState(Mage::getStoreConfig('payment/vtweb1/'),true,
        'New order, waiting for payment.');
    $order->sendNewOrderEmail();
    $order->setEmailSent(true);

    $api_version = Mage::getStoreConfig('payment/vtweb1/api_version');
    $payment_type = Mage::getStoreConfig('payment/vtweb1/payment_types');
    $enable_installment = Mage::getStoreConfig('payment/vtweb1/enable_installment');
    $is_enabled_bni = Mage::getStoreConfig('payment/vtweb1/enable_installment_bni');
    $is_enabled_mandiri = Mage::getStoreConfig('payment/vtweb1/enable_installment_mandiri');

    Veritrans_Config::$isProduction =
        Mage::getStoreConfig('payment/vtweb1/environment') == 'production'
        ? true : false;

    Veritrans_Config::$serverKey =
        Mage::getStoreConfig('payment/vtweb1/server_key_v2');
    
    Veritrans_Config::$is3ds =
        Mage::getStoreConfig('payment/vtweb1/enable_3d_secure') == '1'
        ? true : false;

    Veritrans_Config::$isSanitized =
        Mage::getStoreConfig('payment/vtweb1/enable_sanitized') == '1'
        ? true : false; 
    ...
    if ($current_currency != 'IDR') {
      $conversion_func = function ($non_idr_price) {
          return $non_idr_price *
              Mage::getStoreConfig('payment/vtweb1/conversion_rate');
        };
    ...
    if (Mage::getStoreConfig('payment/vtweb1/enable_indomaret') == '1') {
      $list_enable_payments[] = 'cstore';
    }
    if (Mage::getStoreConfig('payment/vtweb1/enable_dompetku') == '1') {
    ...
    if ($enable_installment == 'allProducts') {
      ...
      if ($is_enabled_bni == 1) {
        $bni_term = Mage::getStoreConfig('payment/vtweb1/installment_bni_term');
      ...
      if ($is_enabled_mandiri == 1) {
        $mandiri_term = Mage::getStoreConfig('payment/vtweb1/installment_mandiri_term');
    ...
    try {
        Mage::log('$payloads:'.print_r($payloads,true),null,'vtweb1_payloads.log',true);
      $redirUrl = Veritrans_VtWeb::getRedirectionUrl($payloads);
      
      if ($isWarning) {
        $this->_getCheckout()->setMsg($redirUrl);        
        $this->_redirectUrl(Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_LINK) . 'vtweb1/paymentwarning/warning/message/1');
      }
      else if (($totalPrice < 500000) && ($isInstallment)) {
        $this->_getCheckout()->setMsg($redirUrl);        
        $this->_redirectUrl(Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_LINK) . 'vtweb1/paymentwarning/warning/message/2');
      }
      else {
      ...
      Mage::log('error:'.print_r($e->getMessage(),true),null,'vtweb1.log',true);
    }
  }
...
//                $this->cancelAction();
            ...
            Mage_Core_Controller_Varien_Action::_redirect('vtweb1/payment/reorder', array('_secure'=>true));
        }
        ...
        if($data->transaction_status == Veritrans_Vtweb1_Model_Standard::TRX_STATUS_SETTLEMENT) {
            Mage::getSingleton('checkout/session')->unsQuoteId();
            Mage_Core_Controller_Varien_Action::_redirect('checkout/onepage/success', array('_secure'=>true));
        } else {
            $order = Mage::getModel('sales/order')->loadByIncrementId($orderId);
  ...
  public function notificationAction() {
    ...
    Veritrans_Config::$isProduction =
        Mage::getStoreConfig('payment/vtweb1/environment') == 'production' ? true : false;
    Veritrans_Config::$serverKey = Mage::getStoreConfig('payment/vtweb1/server_key_v2');
    $notif = new Veritrans_Notification();
    Mage::log('get status result'.print_r($notif,true),null,'vtweb1.log',true);
...
//     $order->sendOrderUpdateEmail(true, 'Thank you, your payment is pending.');
  ...
  public function convert_country_code( $country_code ) {
    ...
    {
	  //.../vtweb1/payment/splitInvoice?id=100000157
