...
$installer->getConnection()->addColumn(
        $installer->getTable('pointofsale'), 'group', "varchar(60)"
);
...
if (version_compare(Mage::getVersion(), '1.4.0', '<')) {
    $installer->getConnection()->addColumn(
            $installer->getTable('sales_order'), 'assignation_stock', 'VARCHAR(5000)'
    );

    $installer->getConnection()->addColumn(
            $installer->getTable('sales_order'), 'assignation_warehouse', 'VARCHAR(5000)'
    );
    ...
    $installer->getConnection()->addColumn(
            $installer->getTable('sales_flat_order'), 'assignation_stock', 'VARCHAR(5000)'
    );
    $installer->getConnection()->addColumn(
            $installer->getTable('sales/order_grid'), 'assignation_stock', 'VARCHAR(5000)'
    );

    $installer->getConnection()->addColumn(
            $installer->getTable('sales_flat_order'), 'assignation_warehouse', 'VARCHAR(5000)'
    );
    $installer->getConnection()->addColumn(
            $installer->getTable('sales/order_grid'), 'assignation_warehouse', 'VARCHAR(5000)'
    );
}
