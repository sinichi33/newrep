...
if (version_compare(Mage::getVersion(), '1.5.0', '>=')) {
    ...
    if (Mage::getSingleton('core/resource')->getConnection('core_write')->isTableExists($table)) {
        $installer->run("INSERT INTO `{$this->getTable('pointofsale')}` (`place_id`,`store_code`,`name`,`main_phone`,`address_line_1`,`address_line_2`,`city`,`state`,`postal_code`,`country_code`,`hours`,`description`,`longitude`,`latitude`,`status`,`image`,`customer_group`,`order`,`store_id`)
                    SELECT `place_id`,`store_code`,`name`,`main_phone`,`address_line_1`,`address_line_2`,`city`,`state`,`postal_code`,`country_code`,`hours`,`description`,`longitude`,`latitude`,`status`,`image`,`customer_group`,`order`,(SELECT GROUP_CONCAT(store_id) FROM `{$this->getTable('localstores_place_store')}` WHERE place_id={$this->getTable('localstores_place')}.place_id) FROM `{$this->getTable('localstores_place')}`;");
...
if (strpos($_SERVER['HTTP_HOST'], "wyomind.com")) {
    $installer->run(
            "
        insert into `pointofsale`(`place_id`,`store_code`,`name`,`main_phone`,`address_line_1`,`address_line_2`,`city`,`state`,`postal_code`,`country_code`,`hours`,`description`,`longitude`,`latitude`,`status`,`image`,`customer_group`,`store_id`,`order`,`email`) values (541,'FR-75','France Paris Store','123456788','86 Avenue des Champs-Elysées',null,'Paris','01','75000','FR','{\"Monday\":{\"from\":\"08:00\",\"to\":\"20:00\"},\"Tuesday\":{\"from\":\"08:00\",\"to\":\"20:00\"},\"Wednesday\":{\"from\":\"08:00\",\"to\":\"20:00\"},\"Thursday\":{\"from\":\"08:00\",\"to\":\"20:00\"},\"Friday\":{\"from\":\"08:00\",\"to\":\"20:00\"},\"Saturday\":{\"from\":\"08:00\",\"to\":\"12:00\"}}','Magento extension','2.303120 ','48.871203',1,'','0,1,2,3,4','3',1,'contact@website.com')
