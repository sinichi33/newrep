...
class Icube_Import_IndexController extends Mage_Core_Controller_Front_Action{
    ...
    public function importproductsAction() {
		...
		foreach (glob($location_path.DS.'AH_*.csv') as $filepath) {
			...
			try{
				...
				//get list of SKUs for enable multistock after importing
				...
				$skus = array_diff($skus, array('sku'));
    ...
    public function importccpAction() {
    ...
    public function importcpiAction() {
    ...
    protected function enableMultiStock($skus){
    	$products = Mage::getModel('catalog/product')->getCollection()->addAttributeToFilter('sku', array( 'in' => $skus));
    	foreach ($products as $product) {
    		if(!$product){
    			Mage::log("Error: SKU ".$sku." not found. So, multistock can not be enabled for this sku.",null,"import.log");
    			continue;
    		}
    		$stock = Mage::getModel('advancedinventory/item')->loadByProductId($product->getId());
	        $status = 1;
	        if($stock->getId() == null) {
	            try {
