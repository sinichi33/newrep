 ...
 * 
 * Description:
 * - This Js belongs to Youama ext, overriden from rwd/default
 * - Modify which link that will trigger the login box to be shown
 * - Add logic to allow register ajax box by create var register_is_ajax. In this project, we set it as false.
 * - Add setErrorMain() to show error at the top of the form
 * - disable this line to make sure only element saved in var trigger_link will open the login box:
 *   //Open login window by back-link on customer/account/forgotpassword
 * - Change element which will be set the opacity dynamically
 * - Change animate show/close box to fadeIn/fadeOut
 * - hide other rollover box in header while showing login box
 */
...
(function($) {
    $.fn.youamaAjaxLogin = function(options) {
        ...
        var register_is_ajax = false;
        var trigger_link = jQuery('header .account-link .links a');
        ...
        function animateShowWindow(windowName) {
            //icube: hide other rollover box in header
        ...
        function animateCloseWindow(windowName, quickly, closeParent) {
            if (opts.stop != true){
                if (quickly == true) {
                    ...
                    $('.youama-' + windowName + '-window').fadeOut(function() {
                        if (closeParent) {
                            $('#header-account').removeClass('skip-active');
                        }
                    });
        ...
        function setError(errors, windowName) {
            ...
            $('.youama-' + windowName + '-window .youama-ajaxlogin-error:not(.main)')
                .fadeIn();
        }

        /**
         * Custom Error messages, to show error at the top of the form.
         * @param string errors Comma separated.
         * @param string windowName
         */
        function setErrorMain(errors, windowName) {
            $('.youama-' + windowName + '-window .youama-ajaxlogin-error')
                .text('');
            $('.youama-' + windowName + '-window .youama-ajaxlogin-error')
                .hide();
            ...
            for (var i = 0; i < length; i++) {
                ...
                mainError += '<div class="err-' + errorArr[i] + '">' + errorText + '</div>';
            }

            $('.youama-' + windowName + '-window .youama-ajaxlogin-error.main').html(mainError)
                .fadeIn();
        }
